...
const BlogPostSingle = ({ params }: { params: Params; }) => {
    ...
    const [blogPost, setBlogPost] = useState<Content.ProjectspostDocument[] | null>(null);
    ...
    const [post, setPost] = useState<Content.ProjectspostDocument<string> | null>(null); // Initialize state

    useEffect(() => {
        const fetchBlogPost = async () => {
            if (uid) {
                const client = createClient();
                const fetchedPost = await client.getByUID('projectspost', uid);
                setPost(fetchedPost);
            }
        };
            ...
            >
                <div className="container">
                    ...
                    <div ref={componentRef} className="pt-10">
                        {blogPost && <ProjectPost slice={blogPost.map((item) => item)} />}
                    </div>
                </div>
            </section>
        </>
